...
/**
 *
 * @author Mariusz D?browski
 */
public final class CornersDetector
{
   ...
   */
  public CornersDetector(BitMatrix image, int initSize, int x, int y, int targetMatrixSize) throws NotFoundException {
    this.image = image;
    ...
    upInit = y - halfsize;
    downInit = y + halfsize;
    this.targetMatrixSize = targetMatrixSize;
  ...
  private ResultPoint[] findCorners(int right, int left, int down, int up) throws NotFoundException {
    ...
    float xCorrection = (topRightCorner.getX() - topLeftCorner.getX()) / (2 * targetMatrixSize);
    float yCorrection = (bottomRightCorner.getY() - topRightCorner.getY()) / (2 * targetMatrixSize);
